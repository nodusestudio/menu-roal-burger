    ...
    activeCategoryMeta
        ...
        .forEach((category) => {
            ...
            if (catNormalizada.includes("SALCHIPAPA")) {
                ...
                if (contenedorMenu) contenedorMenu.innerHTML = '';
                const salchiProds = latestProducts.filter(p => normalizeCategoryKey(p.categoria || '').includes('salchipapa'));
            ...
            if (catNormalizada.includes("BEBIDAS") || catNormalizada.includes("ADICIONALES")) {
                const contenedorMenu = document.getElementById("products-container") || document.querySelector(".products-grid") || document.getElementById("categoryProductsPanel") || document.querySelector(".menu-images-container");
                if (contenedorMenu) contenedorMenu.innerHTML = '';
                const bebidaProds = latestProducts.filter(p => {
                    const cat = normalizeCategoryKey(p.categoria || '');
                    return cat.includes('bebida') || cat.includes('adicional');
                });
...
function renderFeaturedCards(carousel, items) {
        ...
        const rawItems = Array.isArray(effectiveItems) && effectiveItems.length ? effectiveItems : [];
        const resolvedItems = rawItems.map((item) => {
            ...
            const safeCategory = explicitCategory
                || (normalizedName.includes('combo burger') || normalizedName.includes('combo perro')
                    ? 'COMBOS CON PAPAS Y BEBIDA'
                    : 'COMBOS MIXTOS');
            const orderImageSrc = normalizeImageAssetPath(item?.order_image_url || displaySrc);
...
function openMenuModal() {
    ...
    const catMeta = (Array.isArray(allCategoryMeta) ? allCategoryMeta : [])
        .concat(Array.isArray(activeCategoryMeta) ? activeCategoryMeta : []);
    modal.querySelectorAll('.menu-section[data-section-name]').forEach(section => {
        const sectionKey = normalizeCategoryKey(section.dataset.sectionName || '');
        if (!sectionKey || sectionKey === normalizeCategoryKey('PORTADA')) return;
        const matched = catMeta.find(c => normalizeCategoryKey(c?.name || '') === sectionKey);
        if (matched?.image_url) {
            const img = section.querySelector('img.menu-image');
            if (img) img.src = matched.image_url;
        }
    });
...
function renderHomeCategoryCards() {
    ...
    if (!categories || categories.length === 0) {
        if (!container.querySelector('.combos-carousel-section')) {
            container.innerHTML = '<p class="home-loading-msg">Cargando categorías...</p>';
        }
        return;
    }

    const allProducts = (latestProducts || []).filter(p => String(p.estado || '').trim() !== 'paused');
    const normKey = p => normalizeCategoryKey(String(p.categoria || ''));

    container.innerHTML = '';
    let sectionsAdded = 0;

    categories.forEach(cat => {
        const key = cat.key || normalizeCategoryKey(cat.name || '');
        const products = allProducts.filter(p => normKey(p) === key);
        if (!products.length) return;

        const section = document.createElement('section');
        section.className = 'combos-carousel-section';

        // Header: título + "Ver todo"
        const header = document.createElement('div');
        header.className = 'home-cat-section-header';

        const titleEl = document.createElement('h3');
        titleEl.className = 'combos-section-title';
        titleEl.textContent = cat.name;

        const verTodoBtn = document.createElement('button');
        verTodoBtn.type = 'button';
        verTodoBtn.className = 'home-cat-all-btn';
        verTodoBtn.textContent = 'Ver todo →';
        verTodoBtn.addEventListener('click', () => openCategoryDetail(cat));

        header.appendChild(titleEl);
        header.appendChild(verTodoBtn);
        ...
        products.forEach(product => {
            ...
            const priceRaw = resolveProductDisplayPrice(product);
            ...
            if (priceRaw) {
                ...
                priceEl.className = 'card-product-price';
                priceEl.textContent = `$${Math.round(priceRaw).toLocaleString('es-CO')}`;
            ...
            btn.className = 'mobile-order-btn';
            btn.textContent = '¡Lo Quiero! 🔥';
            btn.addEventListener('click', () => startProductOrderFlow(
                product.nombre || '',
                product.categoria || '',
...
function openPublicVariantesModal(productName, categoryName, buttonId, variantes, baseOptions, replaceItemKey = null) {
    ...
    function _getSabores(v) {
        if (!v.con_bebida || !v.bebida_ref_id || !v.bebida_pres_id) return [];
        const beb = _latestBebidas.find((b) => b.id === v.bebida_ref_id);
        const pres = beb?.presentaciones?.find((p) => p.id === v.bebida_pres_id);
        return pres?.sabores || [];
    }

    const overlay = document.createElement('div');
    overlay.id = 'pubVarModal';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9200;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);';

    const card = document.createElement('div');
    card.style.cssText = 'background:rgba(30,20,10,0.96);border:1.5px solid rgba(255,255,255,0.12);border-radius:20px;padding:22px 20px 18px;width:100%;max-width:420px;max-height:88vh;overflow-y:auto;box-shadow:0 12px 48px rgba(0,0,0,0.55);';
    ...
    const confirmBtn = card.querySelector('#pubVarConfirm');

    function refreshConfirm() {
        if (!selectedVariante) return;
        const sabores = _getSabores(selectedVariante);
        const filled = selectedSabores.filter(Boolean).length;
        const total = selectedSabores.length;
        const ready = !sabores.length || filled === total;
        confirmBtn.disabled = !ready;
        confirmBtn.style.background = ready ? 'linear-gradient(135deg,#ff7a00,#e05000)' : 'rgba(200,100,20,0.35)';
    ...
    function renderSabores(v) {
        const sabores = _getSabores(v);
        ...
        for (let i = 0; i < cant; i++) {
            ...
            if (cant > 1) {
                ...
                lbl.style.cssText = 'font-size:0.68rem;color:rgba(255,200,130,0.4);min-width:20px;text-align:right;flex-shrink:0;';
                lbl.textContent = `${i + 1}.`;
                row.appendChild(lbl);
            }
            ...
            sabores.forEach((s) => {
                ...
                sb.type = 'button';
                sb.style.cssText = 'padding:7px 12px;border-radius:20px;border:1.5px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.07);color:#fff7ef;cursor:pointer;font-size:0.8rem;transition:all .15s;white-space:nowrap;';
                sb.textContent = s;
                sb.addEventListener('click', () => {
                    selectedSabores[slotIdx] = s;
                    slotBtns.forEach((b) => { b.style.background = 'rgba(255,255,255,0.07)'; b.style.borderColor = 'rgba(255,255,255,0.15)'; b.style.color = '#fff7ef'; });
                    sb.style.background = 'rgba(255,122,0,0.3)';
                    sb.style.borderColor = '#ff7a00';
                    sb.style.color = '#ffcc88';
                    refreshConfirm();
                });
                slotBtns.push(sb);
    ...
    variantes.forEach((v) => {
        ...
        btn.addEventListener('click', () => {
            selectedVariante = v;
            selectedSabores = [];
            varBtns.forEach((b) => { b.style.background = 'rgba(255,255,255,0.06)'; b.style.borderColor = 'rgba(255,255,255,0.12)'; });
    ...
    confirmBtn.addEventListener('click', () => {
        if (!selectedVariante || confirmBtn.disabled) return;
        const comment = card.querySelector('#pubVarComment').value.trim();
